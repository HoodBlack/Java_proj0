...
public class Auction {
	...
	private static boolean LoginMenu() {
		...
		try {
			...
			ResultSet rset =stmt.executeQuery("select * from User_");
			String name;
			String pwd;
			while(rset.next())
			{
				...
				if(name.equals(username) && pwd.equals(userpass)){login_token=1;}
	...
	private static boolean SignupMenu() {
		...
		try {
			Statement stmt = conn.createStatement();
			ResultSet rset;
			if(isAdmin.equalsIgnoreCase("y")){rset=stmt.executeQuery("select * from Admin_");}
			else {rset=stmt.executeQuery("select * from user_");}
			String name, pwd;
			//existing user match
			while(rset.next())
			{
		...
		else
		{
			try {
				...
				{
					stmt=conn.prepareStatement("insert into Admin_ values(?,?)");
					stmt.setString(1,new_username);
					stmt.setString(2,userpass);
				}
				else
				{
					stmt=conn.prepareStatement("insert into User_ values(?,?)");
					stmt.setString(1,new_username);
					stmt.setString(2, userpass);
				}
				stmt.executeUpdate();
